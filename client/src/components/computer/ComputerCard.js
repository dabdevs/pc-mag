...
export default function ComputerCard({ computer, display }) {
    ...
    return (
        <div className="my-2 computer-card" role='button' onClick={() => navigate(`/computers/${computer._id}/${computer.name.split(' ').join(' - ')}`)}>
            ...
            {
                display === 'list' ?
                    <div className="d-none d-sm-block bg-white border p-2">
                        <div className='d-flex'>
                            <div className='w-25'>
                                <img className="w-100" height={60} src={computer?.images[0] ? computer.images[0] : '../../img/default-computer-img.jpg'} alt={'Computer image'} />
                            </div>

                            <div className="pt-2 pb-0 px-2 w-75">
                                <div className="h-100">
                                    <h6 className="text-center fw-bolder text-truncate mb-3">{computer.name}</h6>
